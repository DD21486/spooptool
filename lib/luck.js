...
/**
 * Get expected kills from Dink payload: accepts decimal probability (e.g. 0.001 = 1 in 1000)
 * or "1 in N" string. Returns N or null.
 */
function expectedKillsFromRarity(value) {
  ...
  if (typeof value === 'string') return parseRarityToExpectedKills(value);
  return null;
}
...
/**
 * Rarity tiers for luck delta scaling:
 * - 1/100–1/500 (medium): base delta
 * - 1/500–1/1500 (high): +1 for lucky, -1 for unlucky
 * - 1/1500+ (extreme): +2 for lucky, -2 for unlucky
 * expected = expected kills for the drop (e.g. 200 for 1/200).
 * Returns adjusted delta (caller should clamp to [-100, 100]).
 */
function scaleLuckDeltaByRarity(delta, expected) {
  if (delta === 0 || expected == null || expected < 100) return delta;
  let bonus = 0;
  if (expected >= 1500) bonus = delta > 0 ? 2 : -2;
  else if (expected >= 500) bonus = delta > 0 ? 1 : -1;
  const out = delta + bonus;
  return Math.max(-100, Math.min(100, out));
}

module.exports = {
  parseRarityToExpectedKills,
  expectedKillsFromRarity,
  getLuckDelta,
  scaleLuckDeltaByRarity,
};
