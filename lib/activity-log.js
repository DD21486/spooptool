/**
 * Activity log: insert and prune to last 30 entries.
 * Used by api/loot.js (loot) and api/cron/snapshot.js (xp_kc).
 */

async function pruneTo30(sql) {
  try {
    await sql`
      DELETE FROM activity_log
      WHERE id NOT IN (SELECT id FROM activity_log ORDER BY at DESC LIMIT 30)
    `;
  } catch (e) {
...
/**
 * Insert one activity row and prune to 30. Safe if table missing.
 */
...
module.exports = { insertActivity, pruneTo30 };
